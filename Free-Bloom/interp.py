...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    with (torch.no_grad()):
        ...
        idx = [args.interp_idx, args.interp_idx + 1]
        config["inference_config"]["interpolation_step"] = args.interp_step
        interp = validation_pipeline.interpolate_between_two_frames(key_frames[0][:, :, idx], x_T[:, :, idx],
                                                                    rand_ratio=inference_config['diversity_rand_ratio'],
                                                                    where=idx,
                                                                    prompt=[prompt[i] for i in idx],
                                                                    k=args.interp_num,
                                                                    generator=generator,
                                                                    negative_prompt=negative_prompt[0],
                                                                    text_embedding=text_embedding[idx],
                                                                    base_noise=x_base[:, :, idx],
                                                                    **validation_data)
        sample = torch.cat([key_frames[0][:, :, :idx[0]], interp, key_frames[0][:, :, idx[1] + 1:]], dim=2)
        torch.cuda.empty_cache()

        samples.append(sample)
    samples = torch.concat(samples)
    save_videos_per_frames_grid(samples, f'{output_dir}/interp_img_samples', n_rows=6)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interp_idx", type=int, required=True, help="where to interpolate")
    parser.add_argument("--interp_num", type=int, default=2)
